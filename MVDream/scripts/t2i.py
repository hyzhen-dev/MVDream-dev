...
def t2i(model, image_size, prompt, uc, sampler, step=20, scale="7.5", batch_size=8, ddim_eta=0., dtype=torch.float32, device="cuda", camera=None, num_frames=1, sag_scale=0):
    ...
    if '|' in prompt:
        ...
        assert len(prompt) == len(scale) or len(scale) == 1, "scale must be a single value or have the same length as prompt"
    ...
    with torch.no_grad(), torch.autocast(device_type=device, dtype=dtype):
        ...
        samples_ddim, _ = sampler.sample(S=step, conditioning=c_,
                                        ...
                                        eta=ddim_eta, x_T=None, sag_scale=sag_scale)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scale", type=str, default="7.5")
    parser.add_argument("-s", "--sample", action="store_true")
    parser.add_argument("--sag_scale", type=float, default=0.0)
    ...
    images = []
    for j in range(4):
        img = t2i(model, args.size, t, uc, sampler, step=50, scale=args.scale, batch_size=batch_size, ddim_eta=0.0, 
                dtype=dtype, device=device, camera=camera, num_frames=args.num_frames, sag_scale=args.sag_scale)
    ...
    if args.sample:
        ...
        Image.fromarray(images).save(f"results/{args.text}_{args.scale}_{args.sag_scale}.png")
